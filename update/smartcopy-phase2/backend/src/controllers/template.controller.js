...
const templateStorage = multer.diskStorage({
  destination: async (req, file, cb) => {
    ...
    const uploadDir = path.join(__dirname, `../../templates/${category}`);
    try {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: mockTemplates.filter(t => {
        ...
        if (institution && t.institution !== institution) return false;
        if (is_default !== undefined && t.is_default !== (is_default === 'true')) return false;
        return true;
      })
    });
